...
std::tuple<std::string, pair_insertions_t>
parse_input(const std::vector<std::string> &input) {
  ...
  for (const auto &line : input) {
    ...
    if (location != std::string::npos) {
      pair_insertions.emplace(std::make_pair(line[0], line[1]),
                              line[line.size() - 1]);
    } else {
...
int64_t day14::run_part1(const std::vector<std::string> &input) {
  ...
  std::list<char> current_polymer(begin(polymer_template),
                                  end(polymer_template));
  ...
  for (size_t i = 0; i < 40; ++i) {
    ...
    for (const auto &entry : pair_counts_copy) {
      ...
      if (result != pair_insertions.end()) {
        ...
        pair_counts[std::make_pair(entry.first.first, result->second)] +=
            entry.second;
        pair_counts[std::make_pair(result->second, entry.first.second)] +=
            entry.second;
      }
    }
  }
