...
parse_grid(const std::vector<std::string> &input) {
...
find_low_points(const std::vector<std::vector<uint8_t>> &grid) {
  ...
  for (const auto &point : low_points) {
    ...
    while (!queue.empty()) {
      ...
      if (current.first > 0 && grid[current.first - 1][current.second] < 9 &&
          visited.find(std::make_pair(current.first - 1, current.second)) ==
              visited.end()) {
        queue.emplace_back(current.first - 1, current.second);
      }

      if (current.second > 0 && grid[current.first][current.second - 1] < 9 &&
          visited.find(std::make_pair(current.first, current.second - 1)) ==
              visited.end()) {
