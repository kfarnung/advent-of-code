...
std::tuple<int64_t, int64_t> parse_packet(bitstream &bs)
{
    ...
    version_sum += static_cast<int64_t>(version);
    ...
    if (type_id == 4)
    {
        ...
        do
        {
            ...
            value += static_cast<int64_t>(current_group & 0x0F);
    ...
    else
    {
        ...
        case 0:
            // sum
            result = std::accumulate(begin(subpacket_results), end(subpacket_results), 0LL,
                                     [](int64_t total, int64_t current) { return total + current; });
            break;

        case 1:
            // product
            result = std::accumulate(begin(subpacket_results), end(subpacket_results), 1LL,
                                     [](int64_t total, int64_t current) { return total * current; });
            break;
        ...
        case 2:
            // minimum
            result =
                std::accumulate(begin(subpacket_results), end(subpacket_results), std::numeric_limits<int64_t>::max(),
                                [](int64_t total, int64_t current) { return std::min(total, current); });
            break;

        case 3:
            // maximum
            result =
                std::accumulate(begin(subpacket_results), end(subpacket_results), std::numeric_limits<int64_t>::min(),
                                [](int64_t total, int64_t current) { return std::max(total, current); });
            break;
