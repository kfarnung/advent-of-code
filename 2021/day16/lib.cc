  ...
  if (type_id == 4) {
    ...
    case 0:
      ...
      result = std::accumulate(
          begin(subpacket_results), end(subpacket_results), 0LL,
    ...
    case 1:
      // product
      result = std::accumulate(
          begin(subpacket_results), end(subpacket_results), 1LL,
          [](int64_t total, int64_t current) { return total * current; });
      break;

    case 2:
      // minimum
      result = std::accumulate(begin(subpacket_results), end(subpacket_results),
