...
void add_graph_edge(Graph &graph, std::string start, std::string end,
                    int distance) {
...
size_t
day09::calculate_shortest_distance(const std::vector<std::string> &input) {
  auto graph = create_graph(input);

  int min_distance = std::numeric_limits<int>::max();
  for (const auto &element : graph) {
    min_distance =
        std::min(min_distance, find_shortest_distance(graph, element.first));
  }
...
size_t
day09::calculate_longest_distance(const std::vector<std::string> &input) {
  auto graph = create_graph(input);

  int max_distance = std::numeric_limits<int>::min();
  for (const auto &element : graph) {
    max_distance =
        std::max(max_distance, find_longest_distance(graph, element.first));
  }
