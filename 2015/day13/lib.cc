...
int64_t happiness_recurse(HappinessMap &map,
                          std::unordered_set<std::string> visited,
                          std::string first_name, std::string current_name,
  ...
  for (const auto &entry : map[current_name]) {
    ...
    max_value = std::max(
        max_value, happiness_recurse(map, next_visited, first_name, entry.first,
                                     current_total + entry.second));
  }
...
int64_t day13::find_highest_happiness(const std::vector<std::string> &input,
                                      bool include_me) {
