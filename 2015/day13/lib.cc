...
int64_t happiness_recurse(HappinessMap &map, std::unordered_set<std::string> visited, const std::string &first_name,
                          const std::string &current_name, int64_t current_total)
{
    bool found = false;
    ...
    for (const auto &entry : map[current_name])
    {
        ...
        max_value = std::max(
            max_value, happiness_recurse(map, next_visited, first_name, entry.first, current_total + entry.second));
    }
