...
std::tuple<std::vector<Replacement>, std::string>
parse_inputs(const std::vector<std::string> &input) {
  std::regex re(R"(^(\w+) => (\w+)$)");
...
find_inverse_replacements(const std::string &current,
  ...
  for (const auto &replacement : replacements) {
    ...
    if (found != std::string::npos) {
      valid_replacements.emplace_back(found, replacement.second.size(),
                                      replacement.first);
    }
  }

  // Sort so that the latest (and then longest) replacement is first.
  std::sort(begin(valid_replacements), end(valid_replacements),
...
size_t day19::count_molecules(const std::vector<std::string> &input) {
  ...
  for (const auto &replacement : replacements) {
    ...
    while ((found = starting_molecule.find(replacement.first, found)) !=
           std::string::npos) {
      found_molecules.insert(
          starting_molecule.substr(0, found) + replacement.second +
...
size_t day19::find_minimum_replacements(const std::vector<std::string> &input) {
  ...
  while (!stack.empty()) {
    ...
    auto valid_replacements =
        find_inverse_replacements(current.first, replacements);
    if (current.second < valid_replacements.size()) {
      ...
      new_molecule.replace(std::get<0>(replacement), std::get<1>(replacement),
