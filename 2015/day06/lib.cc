...
parse_instructions(const std::vector<std::string> &input) {
  ...
  for (const auto &line : input) {
    ...
    if (std::regex_match(line, sm, dimensions_regex)) {
      parsed_instructions.emplace_back(
          std::make_tuple(sm[1].str(),
                          common::Point2D{common::string_to_int(sm[2].str()),
                                          common::string_to_int(sm[3].str())},
                          common::Point2D{common::string_to_int(sm[4].str()),
...
void process_instructions_part1(std::unordered_map<common::Point2D, bool> &grid,
