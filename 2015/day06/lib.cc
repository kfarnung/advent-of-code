    ...
    for (const auto &line : input)
    {
        ...
        if (std::regex_match(line, sm, dimensions_regex))
        {
            parsed_instructions.emplace_back(std::make_tuple(
                sm[1].str(), common::Point2D{common::string_to_int(sm[2].str()), common::string_to_int(sm[3].str())},
...
void process_instructions_part1(std::unordered_map<common::Point2D, bool> &grid, const std::vector<instruction> &input)
{
...
void process_instructions_part2(std::unordered_map<common::Point2D, int64_t> &grid,
