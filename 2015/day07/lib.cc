...
parse_connections(const std::vector<std::string> &input) {
  std::unordered_map<std::string, Connection> connections;
  std::regex re("^(?:([a-z0-9]+) )\?\?(?:(AND|LSHIFT|NOT|OR|RSHIFT) "
                ")?([a-z0-9]+) -> ([a-z]+)$");

  for (const auto &line : input) {
    std::smatch sm;
    if (std::regex_match(line, sm, re)) {
      connections[sm[4].str()] =
          Connection{sm[1].str(), sm[2].str(), sm[3].str()};
    }
  }
...
uint16_t
get_wire_output(std::unordered_map<std::string, uint16_t> &memo,
  ...
  if (connection.operand.empty()) {
    ...
    value = get_wire_output(memo, connections, connection.a) &
            get_wire_output(memo, connections, connection.b);
...
uint16_t day07::get_wire_output(const std::vector<std::string> &input,
                                const std::string &wire) {
...
uint16_t day07::get_wire_output_part2(const std::vector<std::string> &input,
                                      const std::string &wire) {
  std::unordered_map<std::string, uint16_t> memo;
  auto connections = parse_connections(input);
