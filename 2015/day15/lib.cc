...
int64_t calculate_score(const std::vector<Ingredient> &input,
...
int32_t calculate_calories(const std::vector<Ingredient> &input,
                           const std::vector<int32_t> &counts) {
  int32_t calories = 0;
  for (size_t i = 0; i < input.size(); i++) {
...
int64_t day15::find_best_cookie(const std::vector<std::string> &input,
  ...
  while (sum >= 0) {
    ...
    if (calorie_target <= 0 ||
        calorie_target == calculate_calories(ingredient_list, counts)) {
