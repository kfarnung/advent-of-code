...
class Rect:
    ...
    def point_on_boundary(self, point):
        ...
        return (
            point.coord_x == self.upper_left.coord_x
            or point.coord_x == self.lower_right.coord_x
            or point.coord_y == self.upper_left.coord_y
            or point.coord_y == self.lower_right.coord_y
        )


def _distance_to_all(points, point_to_check):
    return reduce(
        lambda prev, current: prev + current.manhattan_distance(point_to_check), points, 0
    )
...
def run_part2(file_content, distance):
    ...
    return len(
        _find_points_within_distance(
            (Point.from_string(input_str) for input_str in file_content), distance
        )
    )
...
if __name__ == "__main__":
    ...
    def run(input_path):
        ...
        with open(input_path) as input_file:
            ...
            print(f"Part 2: {run_part2(file_content, 10000)}")
