...
class Grid:
    ...
    def __str__(self):
        ...
        return "\n".join(
            "".join(self.ground[(coord_x, coord_y)] for coord_x in range(min_x, max_x + 1))
    ...
    def get_settled_count(self):
        ...
        return reduce(
            lambda prev, current: prev + 1 if current == "~" else prev, self.ground.values(), 0
        )
...
def _parse_clay(file_content):
    ...
    for line in file_content:
        ...
        clay += [
            (first, second) if not flipped else (second, first)
